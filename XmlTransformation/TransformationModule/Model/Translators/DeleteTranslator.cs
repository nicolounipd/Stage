...
{
    public class DeleteTranslator : Translator
    {
        /// <summary>
        /// Ritorna il codice XSLT di un Command delete
        /// </summary>
        /// <param name="command">Command delete del quale si vuole ottenere il codice XSLT</param>
        ...
        public override string GetSpecificTranslation(Command command)
        {
            ...
            if (typeAttr == "attribute")
            {
                ...
                return $"<xsl:template match=\"{inAttr}{predicate}/@{nameAttr}\"/>";
            }
            else // typeAttr = "element"
            {
                string separator = inAttr.EndsWith("/") ? "" : "/";
                return $"<xsl:template match=\"{inAttr}/{nameAttr}{predicate}\"/>";
            }
        }
    }
}
